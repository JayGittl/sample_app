...
describe User do
  ...
  it { should be_valid }                  # sanity check, verifying that @user object is initially valid

  describe "when name is not present" do
    before { @user.name = " " }           # set users name to "blank" 
    it { should_not be_valid }            # check if the result is not valid!
  end
  ...
  describe "when name is too long" do
    before { @user.name = "a" * 51 }      # 51 chars, -> should not be valid...
    it { should_not be_valid }
  end
  ...
  describe "email adress with mixed case" do
    let(:mixed_case_email) { "HerO@KENIa.KeN" }
